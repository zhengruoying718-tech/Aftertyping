...
function getSelectedExportTarget() {
  ...
  const svg = isSound ? soundScore : liveTrace;
  ...
  if (!svg) {
    console.error(`Export target missing for ${label}.`, { currentResultView });
    window.alert("Export failed. Please try again.");
    return null;
  }

  return { svg, label, slug, background };
}

function getExportSvgClone() {
  const target = getSelectedExportTarget();
