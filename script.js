...
function drawTraceItems() {
  ...
  const flushDeletedRun = () => {
    ...
    svgLine(deletedRun.x1, deletedRun.y, deletedRun.x2, deletedRun.y, COLORS.text, { width: 1.4, opacity: 0.44 });
    deletedRun = null;
  };
...
function renderHomeTrace() {
  ...
  const flushDeletedRun = () => {
    if (!deletedRun) return;
    homeSvgLine(deletedRun.x1, deletedRun.y, deletedRun.x2, deletedRun.y, COLORS.text, { width: 1.1, opacity: 0.36 });
    deletedRun = null;
  };
